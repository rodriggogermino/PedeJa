...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($data['acao']) && $data['acao'] === 'atualizar_estado') {
        $id_pedido = $data['id'];
        $estado_atual = $data['estado_atual'];
        $novo_estado = $estado_atual;

        if ($estado_atual == 'Pendente') {
            $novo_estado = 'Em Preparação';
        } elseif ($estado_atual == 'Em Preparação') {
            $novo_estado = 'Entregue';
        }

        if ($novo_estado !== $estado_atual) {
            $stmt = $conn->prepare("UPDATE pedido SET estado = ? WHERE id_pedido = ?");
            $stmt->bind_param("si", $novo_estado, $id_pedido);
            $stmt->execute();
            echo json_encode(['sucesso' => true, 'novo_estado' => $novo_estado]);
        } else {
            echo json_encode(['sucesso' => false, 'mensagem' => 'Estado já está concluído']);
        }
        exit;
    }
}
...
<head>
    ...
    <title>Ver Pedido - PedeJá</title>
    <link rel="stylesheet" href="css/ver_pedidoAdmin.css">
...
<body>
                ...
                <div class="grelha-detalhes">
                    ...
                    <div class="coluna-esquerda">
                        ...
                        <div class="seccao-estado">
                            <span class="texto-estado" id="texto-estado" style="<?php echo $cor_texto; ?>"><?php echo htmlspecialchars($estado); ?></span>

                            <div class="barra-progresso">
                                <div class="passo <?php echo $passo1_class; ?>"></div>
                                <div class="linha <?php echo $linha1_class; ?>"></div>
                                <div class="passo <?php echo $passo2_class; ?>"></div>
                                <div class="linha <?php echo $linha2_class; ?>"></div>
                                <div class="passo <?php echo $passo3_class; ?>"></div>
                            </div>

                            <?php if ($estado !== 'Entregue'): ?>
                                <button class="btn-atualizar" onclick="atualizarEstado(<?php echo $id_pedido; ?>, '<?php echo $estado; ?>')">Atualizar</button>
                            <?php endif; ?>
                        </div>
    ...
    <script>
        function atualizarEstado(id, estadoAtual) {
            fetch('ver_pedidoAdmin.php?id=' + id, {
                method: 'POST',
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({ acao: 'atualizar_estado', id: id, estado_atual: estadoAtual })
            })
            .then(res => res.json())
            .then(data => {
                if (data.sucesso) {
